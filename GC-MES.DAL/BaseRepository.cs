...
namespace GC_MES.DAL
{
    public abstract class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        public T QueryById(object pkValue, bool blUseNoLock = false)
        {
            return DbBaseClient
                .Queryable<T>()
                .With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted)
                .InSingle(pkValue);
        }
        ...
        {
            return DbBaseClient.Queryable<T>().Where(predicate).With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).Any();
        }
        ...
        {
            return await DbBaseClient.Queryable<T>().Where(predicate).With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).AnyAsync();
        }
        ...
        {
            return DbBaseClient.Queryable<T>().With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).Count(predicate);
        }
        ...
        {
            return await DbBaseClient.Queryable<T>().With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).CountAsync(predicate);
        }
        ...
        {
            return DbBaseClient.Queryable<T>().Where(predicate).With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).Sum(field);
        }
        ...
        public async Task<int> GetSumAsync(Expression<Func<T, bool>> predicate, Expression<Func<T, int>> field,
            bool blUseNoLock = false)
        {
            return await DbBaseClient.Queryable<T>().Where(predicate).With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted).SumAsync(field);
        }
            ...
            bool blUseNoLock = false) where T1 : class, new()
        {
            return await DbBaseClient
                .Queryable(joinExpression)
                .WhereIF(whereLambda is not null, whereLambda)
                .Select(selectExpression).With(blUseNoLock ? SqlWith.NoLock : SqlWith.ReadCommitted)
                .FirstAsync();
        }
